// Welcome to quick-mail.js 3.5.5
/**
 * Set local storage.
 *
...
function filter_qm_cc_input() {
	if (!jQuery('#qm-email').length && !jQuery('#qm-cc').length) {
		return;
	}
	var cc = jQuery('#qm-cc').val();
	var userdata = jQuery.trim(cc.replace(/<\/?[^>]+(>|$)/g, "")); // strip tags
	var unspaced = userdata.replace(/ /g, ' '); // replace multiple spaces with single space
	if (unspaced != cc) {
		jQuery('#qm-cc').val(unspaced);
		cc = userdata;
	} // end if has html

	var to = jQuery('#qm-email').val();
	userdata = jQuery.trim(to.replace(/<\/?[^>]+(>|$)/g, ""));
	unspaced = userdata.replace(/ /g, ' ');
	if (userdata != to) {
		jQuery('#qm-email').val(unspaced);
		to = userdata;
	} // end if has html
