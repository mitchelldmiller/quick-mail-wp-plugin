// Hello. This is quick-mail.js 4.0.2
// Updated from: https://github.com/jquery/jquery-migrate/blob/master/warnings.md
/**
 * Set local storage.
 *
 ...
 */
function make_qm_to_select(location, is_cc) {
...
function filter_qm_cc_input() {
	...
	if (to.trim().length == 0 && cc.trim().length == 0) {
		return;
	} // nothing to check
	var commas = [];
	var j = 0;
...
// button.on("click", function () { buttonClick(button, popup); });
	...
	jQuery('#show_quick_mail_commenters').on('click', function() {
	...
	jQuery('#do_not_show_quick_mail_users').on('click', function() {
		if (jQuery(this).prop('checked')) {
			enable_qm_verify(true);
		} else {
			enable_qm_verify(false);
	...
	jQuery('#qm_all_users').on('click', function() {
		if (jQuery(this).prop('checked')) {
			enable_qm_verify(false);
	...
	jQuery('#show_quick_mail_names').on('click', function() {
		if (jQuery(this).prop('checked')) {
			enable_qm_verify(false);
		} else {
			enable_qm_verify(true);
    ...
    if (jQuery('#quick_mail_cannot_reply').length) {
		...
		jQuery('#quick_mail_cannot_reply').on('click', function() {
		   if (jQuery('#quick_mail_cannot_reply').prop('checked')) {
			   jQuery('#show_commenters_row').hide();
			   jQuery('#limit_commenters_row').hide();
			} else {
	...
	if (jQuery('#qm-email').length) {
		jQuery('#qm-email').on('keypress', function(event) {
			if (event.keyCode == 13) {
	...
	if (jQuery('#qm-cc').length) {
		jQuery('#qm-cc').on('keypress', function(event) {
			if (event.keyCode == 13) {
				jQuery('#qm-cc').blur();
				return false;
			}
   ...
   jQuery('#qm-first').on('click', function() {
	   clear_qm_msgs();
	});
   jQuery('#qm_bcc').on('click', function() {
	   var cmsg = jQuery(this).is(':checked') ? jQuery('#qm_say_bcc').val() : jQuery('#qm_say_cc').val();
	   jQuery('#qmcc_label').text(cmsg);
	});
   jQuery('#qm-email').on('click', function() {
	   clear_qm_msgs();
	});
   jQuery('#qm-email').on('change', function() {
	   jQuery('#qm-email').blur();
   });
   jQuery('#qm-email').on('blur', function() {
	   filter_qm_cc_input();
	   if (!jQuery('#qm-validate').is(':visible') && !jQuery('#qm-duplicate').is(':visible') ) {
		jQuery('#qm-cc').focus();
	}
   });
   jQuery('#qm-cc').on('change', function() {
	   jQuery('#qm-cc').blur();
   });
   jQuery('#qm-cc').on('click', function() {
	   clear_qm_msgs();
	});
   jQuery('#qm-cc').on('blur', function() {
	   if (!jQuery('#qm-cc').trim().length) {
		   return true;
	   }
	   filter_qm_cc_input();
	   if (!jQuery('#qm-validate').is(':visible') && !jQuery('#qm-duplicate').is(':visible') ) {
	       jQuery('#qm-subject').focus();
	}
   });
   jQuery('#qm-subject').on('focus', function() {
	   clear_qm_msgs();
	   return true;
   });
   jQuery('#quickmailmessage').on('focus', function() {
	   clear_qm_msgs();
	   return true;
   });
   jQuery('#qm-primary').on('change', function() {
	   return is_qm_email_dup();
   });
   jQuery('#qm-secondary').on('change', function() {
	   return is_qm_email_dup();
   });
   jQuery('#qm-file-first').on('change', function() {
	   if (this.value != '') {
   ...
   jQuery('#qm-second-file').on('change', function() {
	   if (this.value != '') {
   ...
   jQuery('#qm-third-file').on('change', function() {
	   if (this.value != '') {
   ...
   jQuery('#qm-fourth-file').on('change', function() {
	   if (this.value != '') {
   ...
   jQuery('#qm-fifth-file').on('change', function() {
	   if (this.value != '') {
   ...
   jQuery("#Hello").on('submit', function( event ) {
	   clear_qm_msgs();
	   jQuery('#qm-email').blur();
   ...
   if (jQuery('#qm_row').length) {
      ...
      if (jQuery('#save_addresses').val() != 'N' && got_saved_qm_addresses()) {
         ...
         make_qm_to_select('#qm_to_choice', false);
         sort_qm_select('#qm_to_select');
         jQuery('#qm_row').show();
         make_qm_to_select('#qm_cc_choice', true);
